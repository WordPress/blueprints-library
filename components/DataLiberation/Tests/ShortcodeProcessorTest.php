...
class ShortcodeProcessorTest extends TestCase {
	...
	/**
	 * @dataProvider mixed_builder_url_rewrite_provider
	 */
	public function test_rewrites_builder_urls_inside_mixed_block_and_shortcode_markup(
		string $builder,
		string $builder_markup,
		array $rewritable_attributes,
		string $escaped_shortcode,
		int $expected_updated_attributes
	): void {
		$old_origin     = 'https://old.example';
		$new_origin     = 'https://new-and-longer.example/site-migration';
		$before_builder = '<!-- wp:group {"metadata":{"name":"Migration [draft]",'
			. '"source":"https://old.example/block-settings.json"}} -->'
			. '<div class="wp-block-group">'
			. '<!-- wp:paragraph -->'
			. '<p data-source="https://old.example/html-attribute">'
			. 'Before [gallery url="https://old.example/gallery"]</p>'
			. '<!-- /wp:paragraph -->'
			. '<!-- wp:shortcode -->';
		$after_builder = '<!-- /wp:shortcode -->'
			. '<!-- wp:code --><pre class="wp-block-code"><code>'
			. $escaped_shortcode
			. '</code></pre><!-- /wp:code -->'
			. '<!-- wp:html -->'
			. '<style>.card[data-source="https://old.example/css-selector"] {'
			. ' background: url("https://old.example/style.jpg"); }</style>'
			. '<!-- /wp:html -->'
			...
			. 'src="https://old.example/block.jpg" /></figure>'
			. '<!-- /wp:image -->';
		$input     = $before_builder . $builder_markup . $after_builder;
		$processor = new ShortcodeProcessor( $input );

		$this->assertSame(
			$expected_updated_attributes,
			$this->rewrite_shortcode_attribute_origins(
				$processor,
		...
		$expected = $before_builder
			. str_replace( $old_origin, $new_origin, $builder_markup )
			. $after_builder;
		$this->assertSame( $expected, $processor->get_updated_text(), $builder );
	}

	public static function mixed_builder_url_rewrite_provider(): array {
		$old_origin = 'https://old.example';

		return array(
			'Divi nested modules with CSS and bracketed URL data' => array(
				'Divi',
				"[et_pb_section background_image='" . $old_origin
					. "/media/hero%20[wide].jpg?fit=cover&dpr=2'"
					. " custom_css_main_element='.hero[data-state=\"open\"] {"
					. ' background-image: url("' . $old_origin
					. '/css/hero.jpg?x=1&y=2"); content: "]"; }\']'
					. '[et_pb_row][et_pb_column type="4_4"]'
					. '[et_pb_image src="' . $old_origin
					. '/media/card.jpg?size=1200x800" url="' . $old_origin
					. '/landing?next=%5Boffer%5D&campaign=summer"]'
					. '[/et_pb_image][/et_pb_column][/et_pb_row][/et_pb_section]',
				array(
					'et_pb_section' => array( 'background_image', 'custom_css_main_element' ),
					'et_pb_image'   => array( 'src', 'url' ),
				),
				'[[et_pb_image src="' . $old_origin . '/escaped.jpg"]]',
				4,
			),
			'WPBakery nested row with design CSS and video query arrays' => array(
				'WPBakery',
				"[vc_row css='.vc_custom[data-breakpoint=\"lg\"] {"
					. ' background-image: url("' . $old_origin
					. '/row.jpg?x=1&y=2"); }\']'
					. '[vc_column][vc_video link="' . $old_origin
					. '/watch?v=abc&playlist%5B%5D=1" el_aspect="169"]'
					. '[/vc_video][vc_raw_html]JTNDcCUzRVJhdyUzQyUyRnAlM0U='
					. '[/vc_raw_html][/vc_column][/vc_row]',
				array(
					'vc_row'   => array( 'css' ),
					'vc_video' => array( 'link' ),
				),
				'[[vc_video link="' . $old_origin . '/escaped.mp4"]]',
				2,
			),
			'Avada container with image, video, and button URLs' => array(
				'Avada',
				'[fusion_builder_container background_image="' . $old_origin
					. '/hero.jpg?width=1600&quality=80" video_webm="'
					. $old_origin . '/hero.webm"]'
					. '[fusion_builder_row][fusion_builder_column type="1_1"]'
					. '[fusion_button link="' . $old_origin
					. '/offer?utm_source=builder&next=%5Bcta%5D" title="A ] title"]'
					. 'Offer[/fusion_button][/fusion_builder_column]'
					. '[/fusion_builder_row][/fusion_builder_container]',
				array(
					'fusion_builder_container' => array(
						'background_image',
						'video_webm',
					),
					'fusion_button'            => array( 'link' ),
				),
				'[[fusion_button link="' . $old_origin . '/escaped"]]',
				3,
			),
			'Oxygen JSON options with arrays and repeated URLs' => array(
				'Oxygen',
				"[ct_section ct_options='{\"selector\":\"section-1\",\"original\":{"
					. '"background-image":"' . $old_origin
					. '/background.jpg?x=1&y=2","overlay-image":"' . $old_origin
					. '/overlay.svg#mask"},"breakpoints":[480,768]}' . "']"
					. "[ct_link_text ct_options='{\"selector\":\"link-2\",\"url\":\""
					. $old_origin
					. "/path?arr%5B%5D=1&arr%5B%5D=2\"}']Link[/ct_link_text]"
					. '[/ct_section]',
				array(
					'ct_section'   => array( 'ct_options' ),
			...
			'Enfold Avia layout with section, pattern, image, and manual link' => array(
				'Enfold',
				"[av_section min_height='custom' src='" . $old_origin
					. "/hero.jpg?crop=[center]&fit=cover' overlay_pattern='"
					. $old_origin . "/pattern.png']"
					. "[av_one_full first][av_image src='" . $old_origin
					. "/image.jpg?x=1&y=2' link='manually," . $old_origin
					. "/landing?ref=hero&next=%5Bcta%5D' attachment='42']"
					. '[/av_image][/av_one_full][/av_section]',
				array(
					'av_section' => array( 'src', 'overlay_pattern' ),
					'av_image'   => array( 'src', 'link' ),
				),
				"[[av_image src='" . $old_origin . "/escaped.jpg']]",
				4,
			),
			'Cornerstone legacy tree with image and lightbox destinations' => array(
				'Cornerstone',
				'[cs_section style="margin: 0; padding: 0;"]'
					. '[cs_row][cs_column type="1/1"]'
					. '[x_image type="thumbnail" src="' . $old_origin
					. '/image.jpg?size=%5Blarge%5D" link="true" href="'
					. $old_origin . '/gallery?slide=1&from=builder" lightbox_thumb="'
					. $old_origin . '/thumb.jpg"]'
					. '[/cs_column][/cs_row][/cs_section]',
				array(
					'x_image' => array( 'src', 'href', 'lightbox_thumb' ),
				),
				'[[x_image src="' . $old_origin . '/escaped.jpg"]]',
				3,
			),
			'Themify nested columns with lightbox and icon links' => array(
				'Themify',
				'[themify_col grid="2-1 first"]'
					. '[themify_button link="' . $old_origin
					. '/page?iframe=true&width=100%&height=100%" '
					. 'style="large purple themify_lightbox"]Button[/themify_button]'
					. '[themify_icon link="' . $old_origin
					. '/social?network=x&next=%5Bprofile%5D" icon="fa-link"]'
					. '[/themify_col]',
				array(
					'themify_button' => array( 'link' ),
					'themify_icon'   => array( 'link' ),
				),
				'[[themify_button link="' . $old_origin . '/escaped"]]',
				2,
			),
		);
	}

	public function test_mixed_block_markup_without_selected_shortcode_urls_is_byte_preserving(): void {
		$old_origin = 'https://old.example';
		$input      = '<!-- wp:cover {"url":"https://old.example/cover.jpg",'
			. '"label":"[gallery]"} -->'
			. '<div class="wp-block-cover" '
			. 'style="background-image:url(https://old.example/cover.jpg)">'
			. '[[et_pb_image src="https://old.example/escaped.jpg"]]'
			. '[et_pb_image data_url="https://old.example/not-selected.jpg"]'
			. '[gallery url="https://old.example/gallery"]'
			. '</div><!-- /wp:cover -->';
		$processor = new ShortcodeProcessor( $input );
	...
	private function rewrite_shortcode_attribute_origins(
		...
		while (
			...
			if ( ! isset( $rewritable_attributes[ $tag ] ) ) {
				continue;
			}
			...
			while ( $processor->next_attribute() ) {
				...
				$value = $processor->get_attribute_value();
				if ( false === strpos( $value, $old_origin ) ) {
					continue;
				}

				$this->assertTrue(
					$processor->set_attribute_value(
						str_replace( $old_origin, $new_origin, $value )
					)
				);
				++$updated_attributes;
			}
